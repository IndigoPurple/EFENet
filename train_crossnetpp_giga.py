...
def save_img(buff, warp_img2_HR, fine_img1_SR, warp_HR, file_dir):
    ...
    for key in keys:
        if key == 'input_LR' or key == 'input_HR':
            for i in range(frame_num):
                ...
                img = Image.fromarray(
                    np.array(temp.numpy()[0].transpose(1, 2, 0) * 255, dtype=np.uint8)[:, :, ::-1])
    ...
    for i in range(frame_num):
        ...
        print(temp.shape)
        img = Image.fromarray(
            np.array(temp.cpu().numpy()[0].transpose(1, 2, 0) * 255, dtype=np.uint8)[:, :, ::-1])
        img.save(file_dir + 'warp_HR_%04d.png' % i)

    warp_img2_HR = np.clip(warp_img2_HR.cpu().numpy(), 0.0, 1.0)
    img = Image.fromarray(np.array(warp_img2_HR[0].transpose(1, 2, 0) * 255, dtype=np.uint8)[:, :, ::-1])
...
if __name__ == '__main__':
    ...
    if dataset_name == 'GIGA':
        data_path_lr = '../../../../fileserver/yaping/data/giga_videosr/LR/'
        data_path_hr = '../../../../fileserver/yaping/data/giga_videosr/HR/'
        composed = transforms.Compose([transforms.ToTensor()])
        dataset_test = MpiiDataset(data_path_lr, data_path_hr, frame_num=frame_num, transform=composed, is_train=True)
        dataset_test = MpiiDataset(data_path_lr, data_path_hr, frame_num=frame_num, transform=composed, is_train=False)
    ...
    if args.with_GAN_loss == 1:
        config['discriminator'] = Discriminator(input_size=(1080, 1920))
