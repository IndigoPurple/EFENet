...
def save_img(buff,sr_img,file_dir):
    ...
    for key in keys:
       ...
       if key == 'input_img1_SR':
           continue
       img = Image.fromarray(np.array(buff[key].numpy()[0].transpose(1,2,0) * 255,dtype=np.uint8)[:,:,::-1])
       img.save(file_dir + key + '.png')

    sr_img = np.clip(sr_img.cpu().numpy(),0.0,1.0)
    img = Image.fromarray(np.array(sr_img[0].transpose(1,2,0) * 255,dtype=np.uint8)[:,:,::-1])
    img.save(file_dir + 'sr.png')



def eval(net,testloader,len_testset,config):
    ...
    for iter_,data in enumerate(testloader):
        ...
        with torch.no_grad():
            ...
            save_img(buff,fine_img1_SR, './%s/%s/'%(config['img_save_path'],seqid[0].strip().split('/')[-2]))
            fine_img1_SR = fine_img1_SR.cpu().numpy()
            for i in range(label_img.shape[0]):
                sum_psnr += psnr(fine_img1_SR[i],label_img[i])
                ssim_ = compare_ssim(cv2.cvtColor(np.array(fine_img1_SR[i]*255.0,dtype=np.uint8).transpose(1,2,0),cv2.COLOR_BGR2GRAY),cv2.cvtColor(np.array(label_img[i]*255.0,dtype=np.uint8).transpose(1,2,0),cv2.COLOR_BGR2GRAY))
                sum_ssim += ssim_
    ...
    print ('------PSNR: %.2f, SSIM: %.2f, so far the best is: %.2f, time: %.2f--------'%(res_psnr,res_ssim,config['best_eval'],time_cost)) 
    ...
    for epoch in range(config['epoch'],5000):
        for iter_,buff in enumerate(trainloader):
            ...
            if discriminator is not None and iter_count % 2 == 0 :
                ...
                w_gan = 0.0001
                #print (loss)
                loss = config['w1'] * loss_1 + loss_2 + config['w2'] * loss_3 + w_gan * loss_g
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
            ...
            if (iter_count + 1) % config['snapshot'] == 0:
                ...
                if discriminator is not None:
                    torch.save(discriminator.state_dict(),config['checkpoints_dir'] + 'D_CP{}.pth'.format(iter_count + 1))
            ...
            if (iter_count + 1) % config['display'] == 0:
                ...
                print ('iter:%d    time: %.2fs / %diters  lr: %.8f  LossR: %.3f %.3f %.3f LossGAN: %.3f %.3f psnr: %.2f'%(iter_count + 1,time_cost,config['display'],config['lr'],loss_count[0],loss_count[1],loss_count[2], loss_d, loss_g, psnr_2))
...
def get_args():
    ...
    parser.add_option('--frame_window_size', dest = 'frame_window_size', default = 2, type = 'int',help = 'frame_window_size')
...
if __name__ == '__main__':
    ...
    if dataset_name=='Vimeo':
        ...
        dataset_train = VimeoDataset(data_path_corrupted, data_path_clean,data_path_MDSR, train_list_file, frame_window_size=frame_window_size, transform=composed)
        dataset_test = VimeoDataset(data_path_corrupted, data_path_clean,data_path_MDSR, test_list_file, frame_window_size=frame_window_size, transform=composed, is_train=False, require_seqid=True)
    ...
    if dataset_name=='MPII':
        ...
        #composed = transforms.Compose([transforms.RandomCrop((128,128)),transforms.ToTensor()])
        composed = transforms.Compose([transforms.ToTensor()])
        dataset_train = MpiiDataset(data_path_corrupted, data_path_clean,data_path_MDSR, train_list_file, min_window_size=3, transform=composed)
        ...
        dataset_test = MpiiDataset(test_path_corrupted, test_path_clean,test_path_MDSR, test_list_file, min_window_size=3, transform=composed, is_train=False, require_seqid=True)
    ...
    if args.with_GAN_loss == 1:
        config['discriminator'] = Discriminator(input_size = (448,640)) 
