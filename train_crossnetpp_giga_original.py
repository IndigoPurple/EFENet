...
from Model import Crossnetpp_Original, Crossnetpp_Multiflow, Crossnetpp_MultiflowFusion
...
def eval(net, testloader, len_testset, config, iter_count = 0):
    ...
    for iter_, data in enumerate(testloader):
        ...
        #    continue
        buff, seqid = data
        print(str(seqid).split('\'')[-2])
        label_img = buff['input_img1_HR'].numpy()
        with torch.no_grad():
            warp_img2_HR, fine_img1_SR = net(buff, require_flow=False)
            save_img(buff, warp_img2_HR, fine_img1_SR, './%s/%04d/%s/' % (config['img_save_path'], iter_count, str(seqid).split('\'')[-2]))
...
def train_net(net, gpu=False, config={}):
    ...
    for epoch in range(config['epoch'], 5000):
        for iter_, buff in enumerate(trainloader):
            ...
            warp_img2_HR, fine_img1_SR, flow_s1_12_1 = net(buff, require_flow=True)
            ...
            if discriminator is not None and iter_count % 2 == 0:
                ...
                w_gan = 0.0001
                # print
                loss = config['w1'] * loss_1 + config['w2'] * loss_3 + w_gan * loss_g
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
            ...
            if (iter_count + 1) % config['display'] == 0:
                ...
                print(
                    'iter:%d    time: %.2fs / %diters  lr: %.8f  LossR: %.3f %.3f  LossGAN: %.3f %.3f psnr: %.2f' % (
                    iter_count + 1, time_cost, config['display'], config['lr'], loss_count[0], loss_count[1],
                    loss_d, loss_g, psnr_2))
...
if __name__ == '__main__':
    ...
    if dataset_name == 'MPII':
        ...
        dataset_test = MpiiDataset(test_path_corrupted, test_path_clean, test_path_MDSR, test_list_file,
                                   frame_num=frame_num, transform=composed, is_train=False, require_seqid=True)
    ...
    if dataset_name == 'GIGA':
        data_path_lr = '../../../../fileserver/yaping/data/giga_videosr_960_512/LR/'
        data_path_hr = '../../../../fileserver/yaping/data/giga_videosr_960_512/HR/'
        composed = transforms.Compose([transforms.ToTensor()])
        dataset_train = GigaDataset(data_path_lr, data_path_hr, frame_num=frame_num, transform=composed, is_train=True)
        dataset_test = GigaDataset(data_path_lr, data_path_hr, frame_num=frame_num, transform=composed, is_train=False)
    ...
    if args.with_GAN_loss == 1:
        config['discriminator'] = Discriminator(input_size=(512, 960))
